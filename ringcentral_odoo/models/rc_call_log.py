...
class RCCallLog(models.Model):
    ...
    def _create_ticket(self, config):
        ...
        # Add rc_escalated field if it exists on the model (from helpdesk_ticket.py)
        ...
        # Attach recording and transcript to ticket
        if self.recording_attachment_id:
            self.recording_attachment_id.copy(
    ...
    def _escalate_ticket(self, ticket, config):
        ...
        # Post note about escalation
        ticket.message_post(
            body=(
                f"<b>⚠️ Priority escalated</b> due to negative call sentiment.<br/>"
                f"Call: {self.name}<br/>"
    ...
    def _build_ticket_description(self):
        ...
        self.ensure_one()
        lines = [
            f"<b>Auto-generated from negative call sentiment</b>",
            f"<br/><br/>",
            ...
            f"<b>Sentiment Score:</b> {self.sentiment_score:.2f}<br/>",
        ]
        if self.transcript:
